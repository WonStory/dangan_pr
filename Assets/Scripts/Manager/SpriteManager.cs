...
public class SpriteManager : MonoBehaviour
{
    ...
    {
        SpriteRenderer t_SpriteRenderer = p_Target.GetComponentInChildren<SpriteRenderer>(); //자식개체거를 가지고옴
        Sprite t_Sprite = Resources.Load("Characters/" + p_SpriteName, typeof(Sprite)) as Sprite; //타입을 확인하고 가져온다음 형변환을 통해 넣어준다.

        if (!CheckSameSprite(t_SpriteRenderer, t_Sprite)) //같지 않을 때만 변경
        {
            Color t_color = t_SpriteRenderer.color;
            t_color.a = 0;
            t_SpriteRenderer.color = t_color; //스프라이트에 넣어줘서 투명해짐
